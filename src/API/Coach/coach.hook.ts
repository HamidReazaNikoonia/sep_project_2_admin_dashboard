...
export const coachKeys = {
  ...
  list: (filters: Record<string, any>) =>
    [...coachKeys.lists(), filters] as const,
}

// Hooks
export const useGetAllCoaches = (params?: {
  page?: number
  limit?: number
  q?: string
}) => {
...
export const useCreateCoach = () => {
  ...
  return useMutation({
    mutationFn: (data: any) => coachAPI.createCoach(data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['coach'] })
      showToast('موفق', 'مربی با موفقیت ایجاد شد', 'success')
    },
    onError: () => {
      showToast('خطا', 'خطا در ایجاد مربی', 'error')
    },
  })
}
